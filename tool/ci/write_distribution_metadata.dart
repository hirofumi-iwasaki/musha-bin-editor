...
void main(List<String> args) {
  ...
  File('README.md')
      .copySync('${output.path}${Platform.pathSeparator}README.md');
  File('README.ja.md')
      .copySync('${output.path}${Platform.pathSeparator}README.ja.md');
  final sourceRevision = _run('git', const ['rev-parse', 'HEAD']);
  final sourceUrl = _run('git', const ['remote', 'get-url', 'origin']);
  final applicationVersion = File('pubspec.yaml')
      .readAsLinesSync()
      .firstWhere((line) => line.startsWith('version:'))
      .substring('version:'.length)
      .trim();
  final flutterVersion = _run(args[2], const ['--version']);
  File('${output.path}${Platform.pathSeparator}SOURCE_AND_BUILD.txt')
      .writeAsStringSync('''Mushagaeshi Binary Editor distribution metadata
Target: ${args[1]}
...
''');
  ...
  final licenses = Directory(
    '${output.path}${Platform.pathSeparator}THIRD_PARTY_LICENSES',
  )..createSync();
  final copied = <String>[];
  ...
  if (packageConfig.existsSync()) {
    ...
    for (final package in config['packages'] as List<dynamic>) {
      ...
      for (final candidate in const ['LICENSE', 'LICENSE.md', 'COPYING']) {
        final license = File(
          '${packageRoot.path}${Platform.pathSeparator}$candidate',
        );
        if (license.existsSync()) {
          final destination = File(
            '${licenses.path}${Platform.pathSeparator}${data['name']}-$candidate',
          );
  ...
  File(
    '${output.path}${Platform.pathSeparator}THIRD_PARTY_NOTICES.txt',
  ).writeAsStringSync(
    '''License texts copied from the package roots resolved by pub are in THIRD_PARTY_LICENSES/. Flutter and Dart notices remain in the complete runtime bundle. pubspec.lock fixes the exact dependency set.
